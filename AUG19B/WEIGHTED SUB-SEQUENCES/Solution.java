...
class Solution {
	public static void main(String args[])throws IOException {
		...
		int n = io.ni(),
		    m = io.ni();
		int[] v = io.nia(26);
		char[] x = io.n().toCharArray(),
		       y = io.n().toCharArray();
		 
		long res = solve(x, y, n, m, v);
		System.out.println(res);
	}
	
	static long solve( char[] X, char[] Y, int m, int n, int[] v) { 
        long L[][] = new long[m+1][n+1]; 
        for (int i=0; i<=m; i++) { 
          for (int j=0; j<=n; j++) { 
            if (i == 0 || j == 0) 
                L[i][j] = 0; 
            else if (X[i-1] == Y[j-1]) 
                L[i][j] = L[i-1][j-1] + v[X[i-1]-'a']; 
            else
                L[i][j] = Meth.max(L[i-1][j], L[i][j-1]); 
          } 
        } 
      return L[m][n]; 
    }
